...
- `conn_mtu()` at `subsys/bluetooth/host/conn.c:618` returns `bt_dev.le.acl_mtu` for LE connections; this is the maximum payload per HCI ACL fragment.
- `get_data_frag()` at `subsys/bluetooth/host/conn.c:168` allocates a zero-copy `fragments` view of at most `conn_mtu(conn)` bytes from the parent `acl_tx_pool` buffer.
- The fragmenting loop at `subsys/bluetooth/host/conn.c:699` computes `frag_len = MIN(conn_mtu(conn), remaining_len)` and sets the HCI Packet Boundary Flag: `BT_ACL_START_NO_FLUSH` (0b10 — "first non-automatically-flushable") for the first fragment, `BT_ACL_CONT` (0b01) for continuation fragments [Core 6.0, Vol 4, Part E, §5.4.2].
- `send_acl()` at `subsys/bluetooth/host/conn.c:534` prepends the `struct bt_hci_acl_hdr` (handle + boundary flag packed by `bt_acl_handle_pack()`) and the H:4 type byte, then calls `bt_send()`.

**Inbound reassembly (controller → host):**

- `hci_acl()` at `subsys/bluetooth/host/hci_core.c:669` receives each HCI ACL fragment from the RX work handler; it pulls the `bt_hci_acl_hdr` and extracts the boundary `flags` field.
- `bt_conn_recv()` at `subsys/bluetooth/host/conn.c:491` dispatches to `bt_acl_recv()` at `subsys/bluetooth/host/conn.c:400`.
